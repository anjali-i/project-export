...
const images = [
  { name: "Netflix", image: NetflixImage },
  { name: "Reddit", image: RedditImage },
  { name: "Amazon", image: AmazonImage },
  { name: "Discord", image: DiscordImage },
  { name: "Spotify", image: SpotImage }
];

const mappedImages = images.map((image) => {
  return (
    <figure key={image.name} className={classes.Sponsors}>
      <Image
        image={image.image}
...
const NetworkSponsors = () => {
  return (
    <div className={`${classes.NetworkSponsors} ${classes.row}`}>
      {mappedImages}
    </div>
  );
};
